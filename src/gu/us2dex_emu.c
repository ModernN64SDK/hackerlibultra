...
static void tmemLoad(Gfx** pkt, u32* imagePtr, s16* imageRemain, s16 drawLines, s16 flagBilerp) {
    ...
    if (iLoadable >= loadLines) { /* If load can be done all at once */
        ...
        if (SubSliceL2 > 0) {
            ...
            tmemLoad_A(pkt, imagePtr2, SubSliceL2, SubSliceY2 * tmemSliceWmax, tmemSliceWmax);
        }
        if (flagSplit) {
            ...
            tmemLoad_A(pkt, imagePtr1B, SubSliceL1, SubSliceY1 * tmemSliceWmax + tmemSH_A, tmemSH_B);
            tmemLoad_A(pkt, imagePtr1A, SubSliceL1, SubSliceY1 * tmemSliceWmax, tmemSH_A);
        }
        if (iLoadable > 0) {
...
void guS2DEmuBgRect1Cyc(Gfx** pkt, uObjBg* bg) {
...

    {
        ...
        */
        ...
        frameX0 = bg->s.frameX;
        frameY0 = bg->s.frameY;
        if ((frameWmax = bg->s.frameW - frameWmax) < 0)
            frameWmax = 0;
        if ((frameHmax = bg->s.frameH - frameHmax) < 0)
            frameHmax = 0;
        frameW -= frameWmax;
        frameH -= frameHmax;
