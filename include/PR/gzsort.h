...
#define gsSPZObject(objA, objB) { (unsigned int)(objA), (unsigned int)(objB) }
...
#define gSPZMoveMem(pkt, id, ofs, len, buffer, flag)                                                                   \
    {                                                                                                                  \
        Gfx* _g = (Gfx*)(pkt);                                                                                         \
        _g->words.w0 = _SHIFTL(G_ZS_MOVEMEM, 24, 8) | _SHIFTL(((len) - 1) >> 3, 15, 9) | _SHIFTL((ofs) >> 3, 6, 9)     \
                       | (id) | (flag);                                                                                \
        _g->words.w1 = (unsigned int)(buffer);                                                                         \
    }

#define gsSPZMoveMem(id, ofs, len, buffer, flag)                                                                       \
    { _SHIFTL(G_ZS_MOVEMEM, 24, 8) | _SHIFTL(((len) - 1) >> 3, 15, 9) | _SHIFTL((ofs) >> 3, 6, 9) | (id) | (flag),     \
      (unsigned int)(buffer) }

/*------------------------------------------------------------------------*/
#define gSPZSetUMem(pkt, umem, size, adrs) gSPZMoveMem((pkt), GZM_USER0, (umem), (size), (adrs), GZF_LOAD)
#define gsSPZSetUMem(umem, size, adrs)     gsSPZMoveMem(GZM_USER0, (umem), (size), (adrs), GZF_LOAD)

#define gSPZGetUMem(pkt, umem, size, adrs) gSPZMoveMem((pkt), GZM_USER0, (umem), (size), (adrs), GZF_SAVE)
#define gsSPZGetUMem(umem, size, adrs)     gsSPZMoveMem(GZM_USER0, (umem), (size), (adrs), GZF_SAVE)

/*------------------------------------------------------------------------*/
#define gSPZSetMtx(pkt, mid, mp) gSPZMoveMem((pkt), (mid), 0, sizeof(Mtx), (mp), GZF_LOAD)
#define gsSPZSetMtx(mid, mp)     gsSPZMoveMem((mid), 0, sizeof(Mtx), (mp), GZF_LOAD)

/*------------------------------------------------------------------------*/
#define gSPZGetMtx(pkt, mid, mp) gSPZMoveMem((pkt), (mid), 0, sizeof(Mtx), (mp), GZF_SAVE)
#define gsSPZGetMtx(mid, mp)     gsSPZMoveMem((mid), 0, sizeof(Mtx), (mp), GZF_SAVE)

/*------------------------------------------------------------------------*/
#define gSPZSetAmbient(pkt, umem, lights) gSPZSetUMem((pkt), (umem), ZSIZE_AMBIENT, (lights))
#define gsSPZSetAmbient(umem, lights)     gsSPZSetUMem((umem), ZSIZE_AMBIENT, (lights))

/*------------------------------------------------------------------------*/
#define gSPZSetDefuse(pkt, umem, lid, lights)                                                                          \
    gSPZSetUMem((pkt), (umem) + ZSIZE_AMBIENT + (lid) * ZSIZE_LIGHT, ZSIZE_DEFUSE, (lights))
#define gsSPZSetDefuse(umem, lid, lights)                                                                              \
    gsSPZSetUMem((umem) + ZSIZE_AMBIENT + (lid) * ZSIZE_LIGHT, ZSIZE_DEFUSE, (lights))

/*------------------------------------------------------------------------*/
#define gSPZSetLookAt(pkt, umem, lid, lookat)                                                                          \
    {                                                                                                                  \
        gSPZSetUMem((pkt), (umem) + ZSIZE_AMBIENT + ((lid) + 0) * ZSIZE_LIGHT, ZSIZE_DEFUSE,                           \
                    ((u32)(lookat)) + sizeof(Light) * 0);                                                              \
        gSPZSetUMem((pkt), (umem) + ZSIZE_AMBIENT + ((lid) + 1) * ZSIZE_LIGHT, ZSIZE_DEFUSE,                           \
                    ((u32)(lookat)) + sizeof(Light) * 1);                                                              \
    }
#define gsSPZSetLookAt(umem, lid, lookat)                                                                              \
    gsSPZSetUMem((umem) + ZSIZE_AMBIENT + ((lid) + 0) * ZSIZE_LIGHT, ZSIZE_DEFUSE,                                     \
                 ((u32)(lookat)) + sizeof(Light) * 0),                                                                 \
        gsSPZSetUMem((umem) + ZSIZE_AMBIENT + ((lid) + 1) * ZSIZE_LIGHT, ZSIZE_DEFUSE,                                 \
                     ((u32)(lookat)) + sizeof(Light) * 1)

/*------------------------------------------------------------------------*/
#define gSPZViewport(pkt, v) gSPZMoveMem((pkt), GZM_VIEWPORT, 0, 16, (v), GZF_LOAD)
#define gsSPZViewport(v)     gsSPZMoveMem(GZM_VIEWPORT, 0, 16, (v), GZF_LOAD)
...
#define GZ_INTR_SPBRK2 (SP_SET_RSPSIGNAL | SP_CLR_TASKDONE | SP_CLR_YIELDED | SP_SET_INTR)

#define gSPZSendMessage(pkt) gDma0p((pkt), G_ZS_SENDSIGNAL, GZ_INTR_SPBRK2, GZ_INTR_SPBRK1)
#define gsSPZSendMessage()   gsDma0p(G_ZS_SENDSIGNAL, GZ_INTR_SPBRK2, GZ_INTR_SPBRK1)
 ...
 */
#define gSPZWaitSignal(pkt, adrs, param) gDma0p((pkt), G_ZS_WAITSIGNAL, (param), (adrs))

#define gsSPZWaitSignal(adrs, param)                                                                                   \
    { (unsigned int)((char*)(adrs) + ((G_ZS_WAITSIGNAL - 0x80) << 24)), (unsigned int)(param) }
...
#define gSPZMultMPMtx(pkt, mid, src, num, dest)                                                                        \
    {                                                                                                                  \
        ...
        _g->words.w1 = _SHIFTL(((num) - 1), 24, 8) | _SHIFTL((src) + 1024, 12, 12) | _SHIFTL((dest) + 1024, 0, 12);    \
    }

#define gsSPZMultMPMtx(mid, src, num, dest)                                                                            \
    { _SHIFTL(G_ZS_MULT_MPMTX, 24, 8) | _SHIFTL((mid), 0, 8),                                                          \
      _SHIFTL(((num) - 1), 24, 8) | _SHIFTL((src) + 1024, 12, 12) | _SHIFTL((dest) + 1024, 0, 12) }
...
#define gsSPZMtxCat(midS, midT, midD)                                                                                  \
    { _SHIFTL(G_ZS_MTXCAT, 24, 8) | _SHIFTL((midS), 0, 8), _SHIFTL((midT), 16, 8) | _SHIFTL((midD), 0, 8) }
...
#define gsSPZXfmLights(mid, umem, lnum)                                                                                \
    { _SHIFTL(G_ZS_XFMLIGHT, 24, 8) | (mid), _SHIFTL((lnum) - 1, 12, 8) | _SHIFTL((umem) + 1024, 0, 12) }
...
#define gSPZLightMaterial(pkt, csrc, nsrc, num, cdest, tdest)                                                          \
    {                                                                                                                  \
        Gfx* _g = (Gfx*)(pkt);                                                                                         \
        _g->words.w0 = _SHIFTL(G_ZS_LIGHTING, 24, 8) | _SHIFTL((csrc) + 1024, 12, 12) | _SHIFTL((nsrc) + 1024, 0, 12); \
        _g->words.w1 = _SHIFTL((num) - 1, 24, 8) | _SHIFTL((cdest) + 1024, 12, 12) | _SHIFTL((tdest) + 1024, 0, 12);   \
    }

#define gsSPZLightMaterial(csrc, nsrc, num, cdest, tdest)                                                              \
    { _SHIFTL(G_ZS_LIGHTING, 24, 8) | _SHIFTL((csrc) + 1024, 12, 12) | _SHIFTL((nsrc) + 1024, 0, 12),                  \
      _SHIFTL((num) - 1, 24, 8) | _SHIFTL((cdest) + 1024, 12, 12) | _SHIFTL((tdest) + 1024, 0, 12) }

#define gSPZLight(pkt, nsrc, num, cdest, tdest) gSPZLightMaterial((pkt), -16, (nsrc), (num), (cdest), (tdest))
#define gsSPZLight(nsrc, num, cdest, tdest)     gsSPZLightMaterial(-16, (nsrc), (num), (cdest), (tdest))
...
#define gSPZMix(pkt, src1, src2, num, factor, type)                                                                    \
    {                                                                                                                  \
        Gfx* _g = (Gfx*)(pkt);                                                                                         \
        _g->words.w0 = _SHIFTL(G_ZS_INTERPOLATE, 24, 8) | _SHIFTL((type), 16, 8) | _SHIFTL((factor), 0, 16);           \
        _g->words.w1 = _SHIFTL((num) / 8 - 1, 24, 8) | _SHIFTL((src1), 12, 12) | _SHIFTL((src2), 0, 12);               \
    }

#define gsSPZMix(src1, src2, num, factor, type)                                                                        \
    { _SHIFTL(G_ZS_INTERPOLATE, 24, 8) | _SHIFTL((type), 16, 8) | _SHIFTL((factor), 0, 16),                            \
      _SHIFTL(((num) - 1) / 8, 24, 8) | _SHIFTL((src1), 12, 12) | _SHIFTL((src2), 0, 12) }

#define gSPZMixS16(pkt, src1, src2, num, factor) gSPZMix((pkt), (src1), (src2), (num), (factor), GZ_INTRP_S16)
#define gSPZMixS8(pkt, src1, src2, num, factor)  gSPZMix((pkt), (src1), (src2), (num), (factor), GZ_INTRP_S8)
#define gSPZMixU8(pkt, src1, src2, num, factor)  gSPZMix((pkt), (src1), (src2), (num), (factor), GZ_INTRP_U8)

#define gsSPZMixS16(src1, src2, num, factor) gsSPZMix((src1), (src2), (num), (factor), GZ_INTRP_S16)
#define gsSPZMixS8(src1, src2, num, factor)  gsSPZMix((src1), (src2), (num), (factor), GZ_INTRP_S8)
#define gsSPZMixU8(src1, src2, num, factor)  gsSPZMix((src1), (src2), (num), (factor), GZ_INTRP_U8)
